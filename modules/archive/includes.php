...
if (!is_admin()) {//is the frontend
    ...
    if ($request_uri == '/profile/') {
        ...
        function go_archive_frontend_scripts($hook){
            ...
            wp_localize_script( 'go_frontend', 'go_delete_temp_archive_nonce', wp_create_nonce( 'go_delete_temp_archive' ) );

            wp_localize_script( 'go_frontend', 'go_zip_archive_nonce', wp_create_nonce( 'go_zip_archive' ) );
        }
    }
    ...
    if($ajax_action === 'go_reset_selected_users' || $ajax_action === 'go_make_user_archive_zip' || $ajax_action === 'go_create_user_list' || $ajax_action === 'go_delete_temp_archive' || $ajax_action === 'go_zip_archive') {
        //always include
        include_once('src/functions.php');
        ...
        add_action( 'wp_ajax_go_reset_selected_users', 'go_reset_selected_users' ); //OK
    ...
    if($page === 'tool_blog_archive'){
        ...
        function go_archive_admin_scripts($hook){
           ...
           // wp_localize_script( 'go_admin_user', 'go_archive_progress_nonce', wp_create_nonce( 'go_archive_progress' ) );
            ...
            wp_localize_script( 'go_admin_user', 'go_reset_selected_users_nonce', wp_create_nonce( 'go_reset_selected_users_nonce' ) );
