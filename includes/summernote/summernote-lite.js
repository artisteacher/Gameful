...
(function (global, factory) {
  ...
  jQuery(document).on('click', function (e) {
      if (!jQuery(e.target).closest('.note-btn-group').length) {
          jQuery('.note-btn-group.open').removeClass('open');
      }
  });
    jQuery(document).on('click.note-dropdown-menu', function (e) {
        jQuery(e.target).closest('.note-dropdown-menu').parent().removeClass('open');
  });
